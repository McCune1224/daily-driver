...
const countGarminFiles = `-- name: CountGarminFiles :one
select count(*) from garmin_fit_files
`

func (q *Queries) CountGarminFiles(ctx context.Context) (int64, error) {
	row := q.db.QueryRow(ctx, countGarminFiles)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
type ListGarminFilesPaginatedParams struct {
	Limit  int32
	Offset int32
}

func (q *Queries) ListGarminFilesPaginated(ctx context.Context, arg ListGarminFilesPaginatedParams) ([]GarminFitFile, error) {
	rows, err := q.db.Query(ctx, listGarminFilesPaginated, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
